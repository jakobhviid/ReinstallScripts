...
- **1Password — load-bearing lessons (don't re-debug from scratch):**
  ...
  - **Vivaldi (`vivaldi-bin`), Zen (`zen-bin`), and Brave Origin (`brave-origin` + `brave-origin-stable`) need explicit allowlist entries.** 1P's hardcoded trusted basenames are only `chrome chromium-browser-privacy msedge brave firefox firefox-bin firefox-esr`. Note "brave" matches `brave-browser`/`brave-browser-stable` but NOT `brave-origin` (the Shields-only Brave variant baked into bazzite-custom alongside brave-browser since 2026-06-05). The cask only writes `flatpak-session-helper`. `run_config_1password` appends all four. Cask file ships without trailing newline — handle when appending.
