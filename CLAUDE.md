...
- `Linux/assets/gnome/shell.dconf` — Snapshot of extension *configs* under `/org/gnome/shell/` (dash-to-panel, dash-to-dock, blur-my-shell, hotedge), loaded by `run_config_gnome_shell`. **Deliberately does NOT include `enabled-extensions`** — which dock/panel a given machine actually runs is a per-machine preference, and re-runs of the installer must not stomp it. The snapshot ships baseline configs for each extension so that whichever one the user enables looks the same across machines. **Regenerate with `just gnome-backup`** — that recipe re-dumps live state and strips bookkeeping keys (prefs-opened, extension-version, rounded-blur-found, settings-version) and per-monitor panel layout (panel-anchors/positions/sizes/lengths/element-positions, which encode this machine's display connector ID and don't transfer). The root `[/]` section is dropped entirely so `enabled-extensions` never gets captured. After tweaking dash-to-panel/dock etc., re-run the recipe and commit.
