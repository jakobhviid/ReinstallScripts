...
**Editing the Brave Origin policy:** edit `assets/brave-origin-policy.json` here AND the byte-identical copy at `Stacks/services/bazzite-build/image/system_files/etc/brave/policies/managed/brave-policy.json` (the image's inline source). Push both. The 10:00 UTC daily `bazzite-build.timer` on Eternium produces a fresh image; `topgrade` on each machine picks it up. `just drift` here flags any deployed key that's missing from or extra in this repo's copy, so divergence between the two source files surfaces quickly.
